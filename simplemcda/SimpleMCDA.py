...
class SimpleMCDA:
    ...
    def simple_mcda(self):
        ...
        # Create An Appropriate Number Of Lists To Hold Raw Data For Each Variable
        for user_def_field in ff_index:
            exec("field_index_" + str(user_def_field) + " = []")


        # Create An Appropriate Number Of Lists To Hold Standardized Data For Each Variable
        for user_def_field in ff_index:
            exec("standardized_fi__" + str(user_def_field) + " = []")


        # Append Raw Data To Appropriate List
        ...
        # --------------------------------------------------------------------------------------------
        # Present The User With Three Methodologies, Maximum Score, Score Range, Or No Standardization
        # --------------------------------------------------------------------------------------------



        # Do Nothing, Values = Values
        if self.dlg.rb_choice_1.isChecked() is True:
            for idx in ff_index:
                exec("raw_data = field_index_" + str(idx))
                exec("for val in raw_data:    standardized_fi__" + str(idx) + ".append((val))")



        # Score Range
        elif self.dlg.rb_choice_2.isChecked() is True:
            for idx in ff_index:
                # Find The Max & Min Value In Each Raw Data List
                exec("max_val = max(field_index_" + str(idx) + ")")
                exec("min_val = min(field_index_" + str(idx) + ")")

                # Loop through Each Value In The Specific Raw Data List & Standardize
                exec("raw_data = field_index_" + str(idx))

                # Calculate Standardized Value
                exec("for val in raw_data:    standardized_fi__" + str(idx) + ".append((val - min_val) / (max_val - min_val))")



        # Maximum Score
        elif self.dlg.rb_choice_3.isChecked() is True:
            for idx in ff_index:
                # Find The Max & Min Value In Each Raw Data List
                exec("max_val = max(field_index_" + str(idx) + ")")
                ...
                # Calculate Standardized Value
                exec("for val in raw_data:    standardized_fi__" + str(idx) + ".append((val / max_val))")



        # Z-Score
        elif self.dlg.rb_choice_4.isChecked() is True:
            for idx in ff_index:
                # Find The Average Value In Each Raw Data List
                exec("avg_val = statistics.mean(field_index_" + str(idx) + ")")

                # Find The Standard Deviation
                exec("st_dev = statistics.stdev(field_index_" + str(idx) + ")")

                # Loop through Each Value In The Specific Raw Data List & Standardize
                exec("raw_data = field_index_" + str(idx))

                # Calculate Standardized Value
                exec("for val in raw_data:    standardized_fi__" + str(idx) + ".append(((val - avg_val) / st_dev))")

        # --------------------------------------------------------------------------------------------
        #                             End Of Main Methodology Choice
        # --------------------------------------------------------------------------------------------

        # Make A Temporary List For Each Index, Will Store Weighted Values
        for user_def_field in ff_index:
            exec("temp_field_idx_" + str(user_def_field) + "= []")

        # Using The Standardized Values, Pull The X Value From Each List, Weight & Compute Composite
        for x in range(int(self.inVector.featureCount())):
            ...
            for user_def_field in ff_index:
                exec("value_" + str(user_def_field) + " = standardized_fi__" + str(user_def_field) + "[" + ph + "]")
        ...
        # Get Field Names In Vector File
        fields = self.inVector.fields()
        field_names = []
        for field in fields:
            field_names.append(str(field.name()))
        ...
        # Create A New Field Based On The Method Chosen
        if self.dlg.rb_choice_1.isChecked() is True:

            try:
                # Delete Previous
                del_index = field_names.index("No_Stdr")
            ...
            # Append A New Column
            provider.addAttributes([QgsField("No_Stdr", QVariant.Double)])
            lyrOutput.updateFields()
            lyrOutput.commitChanges()

        if self.dlg.rb_choice_2.isChecked() is True:
            try:
                # Delete Previous
                del_index = field_names.index("Scr_Rng")
                provider.deleteAttributes([del_index])
                lyrOutput.updateFields()
            ...
            # Append A New Column
            provider.addAttributes([QgsField("Scr_Rng", QVariant.Double)])
            lyrOutput.updateFields()
            lyrOutput.commitChanges()

        if self.dlg.rb_choice_3.isChecked() is True:
            try:
                # Delete Previous
                del_index = field_names.index("Max_Scr")
                provider.deleteAttributes([del_index])
                lyrOutput.updateFields()
            ...
            # Append A New Column
            provider.addAttributes([QgsField("Max_Scr", QVariant.Double)])
            lyrOutput.updateFields()
            lyrOutput.commitChanges()

        if self.dlg.rb_choice_4.isChecked() is True:
            try:
                # Delete Previous
                del_index = field_names.index("Z_Scr")
                provider.deleteAttributes([del_index])
                lyrOutput.updateFields()
            ...
            # Append A New Column
            provider.addAttributes([QgsField("Z_Scr", QVariant.Double)])
            lyrOutput.updateFields()

        # Add Composite Values To New Field, Always The Last One
        fields = self.inVector.fields()
        field_names = []
        for field in fields:
            field_names.append(str(field.name()))
        ...
        last_field_idx = field_names.index(last_field)

        # Update Data
        it = lyrOutput.getFeatures()
        lyrOutput.startEditing()

        counter_c = 0
        for feat in it:
            lyrOutput.changeAttributeValue(feat.id(), last_field_idx, composite_list[counter_c])
            counter_c += 1
